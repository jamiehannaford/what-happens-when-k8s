...
To improve performance, kubectl also [caches the discovered API resources](https://github.com/kubernetes/kubernetes/blob/7650665059e65b4b22375d1e28da5306536a12fb/pkg/kubectl/cmd/util/factory_client_access.go#L117) to the `~/.kube/cache` directory. If you want to see this API discovery in action, try deleting that directory and running a command with the `-v=9` flag. You'll see all the HTTP requests which are trying to find those API versions. There are lots!
